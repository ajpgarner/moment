classdef MakeMatrixSystemTest < NPATKTestBase
    ...
    methods (Test, TestTags={'Error'})
        function Error_NoInputs(testCase)
            function no_in()             
               ref_id = npatk('make_matrix_system');
            end
        ...
        function Error_MissingOperators(testCase)
         ...
         function Error_BadInputsMix(testCase)
            function bad_in()             
               ref_id = npatk('make_matrix_system', 5, ...
                              'parties', 2, ...
                              'operators', 3);
            end
            testCase.verifyError(@() bad_in(), 'npatk:bad_param');
         ...
         function Error_BadInputs1(testCase)
            function bad_in()             
               ref_id = npatk('make_matrix_system', -1, 1);
            end
            testCase.verifyError(@() bad_in(), 'npatk:negative_value');
         end
         ...
         function Error_BadInputs3(testCase)
            function bad_in()             
               ref_id = npatk('make_matrix_system', 1, 1, -1);
            end
            testCase.verifyError(@() bad_in(), 'npatk:negative_value');
         end
         ...
         function Error_TooManyInputs(testCase)
            function bad_in()             
               ref_id = npatk('make_matrix_system', 1, 1, 1, 1, 1);
            end
