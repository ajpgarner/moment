...
namespace Moment::Tests {
    ...
    inline void compare_unique_sequences(const SymbolicMatrix &theMM, std::initializer_list<unique_seq_brace_ref> reference) {
        ...
        ASSERT_NE(iter, theMM.symbols.end()) << " Level = " << mmPtr->Index;
        EXPECT_EQ(&(*iter), &theMM.symbols[0]) << " Level = " << mmPtr->Index;
        ...
        EXPECT_EQ(theMM.symbols[0].sequence_conj(), OperatorSequence::Zero(theMM.context))
                << " Level = " << mmPtr->Index;
        EXPECT_TRUE(theMM.symbols[0].is_hermitian()) << " Level = " << mmPtr->Index;
        ++iter;

        // 1 is always ID
        ASSERT_NE(iter, theMM.symbols.end()) << " Level = " << mmPtr->Index;
        EXPECT_EQ(&(*iter), &theMM.symbols[1]) << " Level = " << mmPtr->Index;
        ...
        EXPECT_EQ(theMM.symbols[1].sequence_conj(), OperatorSequence::Identity(theMM.context))
                << " Level = " << mmPtr->Index;
        EXPECT_TRUE(theMM.symbols[1].is_hermitian())  << " Level = " << mmPtr->Index;
        ++iter;

        size_t index = 2;
        for (const auto& ref_seq : reference) {
            ASSERT_NE(iter, theMM.symbols.end()) << " Level = " << mmPtr->Index << ", index = " << index;
            EXPECT_EQ(&(*iter), &theMM.symbols[index]) << " Level = " << mmPtr->Index
                                                       << ", index = " << index;
            EXPECT_EQ(iter->sequence(), ref_seq.fwd) << " Level = " << mmPtr->Index << ", index = " << index;
            EXPECT_EQ(iter->sequence_conj(), ref_seq.rev) << " Level = " << mmPtr->Index << ", index = " << index;
            EXPECT_EQ(iter->is_hermitian(), ref_seq.herm) << " Level = " << mmPtr->Index << ", index = " << index;
            ++index;
            ++iter;
        }

        EXPECT_EQ(index, 2 + reference.size()) << " Level = " << mmPtr->Index;
        EXPECT_EQ(iter, theMM.symbols.end()) << " Level = " << mmPtr->Index;
    }
}
