/**
 * get_symbol_table.cpp
 * 
 * Copyright (c) 2022 Austrian Academy of Sciences
 */
#include "get_symbol_table.h"
...
namespace NPATK::mex::functions {
    ...
    GetSymbolTableParams::GetSymbolTableParams(matlab::engine::MATLABEngine &matlabEngine, SortedInputs &&inputs) {
    ...
    GetSymbolTable::GetSymbolTable(matlab::engine::MATLABEngine &matlabEngine, StorageManager& storage)
            : MexFunction(matlabEngine, storage, MEXEntryPointID::GetSymbolTable, u"get_symbol_table") {
    ...
    GetSymbolTable::transform_inputs(std::unique_ptr<SortedInputs> inputPtr) const {
        auto& input = *inputPtr;
        auto output = std::make_unique<GetSymbolTableParams>(this->matlabEngine, std::move(input));
    ...
    void GetSymbolTable::operator()(IOArgumentRange output, std::unique_ptr<SortedInputs> inputPtr) {
        auto& input = dynamic_cast<GetSymbolTableParams&>(*inputPtr);
        ...
        if (output.size() >= 1) {
            switch (input.output_mode) {
                case GetSymbolTableParams::OutputMode::AllSymbols:
                    output[0] = export_symbol_table_struct(this->matlabEngine, context, symbolTable);
                    break;
                case GetSymbolTableParams::OutputMode::FromId:
